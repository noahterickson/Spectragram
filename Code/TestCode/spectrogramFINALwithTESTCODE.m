...
function spectrogramFINAL
        ...
        windowType = 'hanning';
        selectedPlot = 'normal';
        spectragramRUN(fs, WIND, colOption,windowType,selectedPlot) %Call the function to run
    function spectragramRUN(fs, WIND, colOption,windowType,selectedPlot)
        close all %closes all open figures
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        %%%%%%%%%%%%%%%%%%%%%%%
        windowGroup = uibuttongroup('parent',cPanel,'Title','Window Type',...
            'position',[.5 .6 .45 .15]);
        set(windowGroup,'SelectionChangeFcn',@windowSelect); %I can't set this in the actual windowgroup initialization.
        rHanning = uicontrol('parent',windowGroup,'style','radiobutton',...
            'units','normalized','position',[.1 .7 .9 .2],'string','Hanning');
        rBlackman = uicontrol('parent',windowGroup,'style','radiobutton',...
            'units','normalized','position',[.1 .4 .9 .2],'String','Blackman');
        rFlattop = uicontrol('parent',windowGroup,'style','radiobutton',...
            'units','normalized','position',[.1 .1 .9 .2],'String','Flat Top');
        %%%%%%%%%%%%%%%%%%%%%%%
        %%Plot type selection%%
        %%%%%%%%%%%%%%%%%%%%%%%
        plotGroup = uibuttongroup('parent',cPanel,'Title','Window Type',...
            'position',[.5 .4 .45 .15]);
        set(plotGroup,'SelectionChangeFcn',@plotSelect); %I can't set this in the actual windowgroup initialization.
        pPower = uicontrol('parent',plotGroup,'style','radiobutton',...
            'units','normalized','position',[.1 .7 .9 .2],'string','Power');
        pLog = uicontrol('parent',plotGroup,'style','radiobutton',...
            'units','normalized','position',[.1 .4 .9 .2],'String','Log Power');
        pSquared = uicontrol('parent',plotGroup,'style','radiobutton',...
            'units','normalized','position',[.1 .1 .9 .2],'String','Squared Power');
        ...
        while x == 1 %Infinite loop, CTRL+C to stop
            ...
            %not needed%dataArray = circshift(dataArray,[round(hopSize) 1]); %use circle shift to move the aray
            %DETERMINE WHAT TO PLOT
            switch selectedPlot
                case 'Power'
                    dataMatrix = dataMatrix;
                case 'Log Power'
                    dataMatrix = abs(log(dataMatrix));
                case 'Squared Power'
    ...
    function fsBoxCallback(obj,event)
        ...
        spectragramRUN(fs, WIND, colOption,windowType,selectedPlot);
    end
    ...
    function refreshButtonCallback(obj, event)
        spectragramRUN(fs, WIND, colOption,windowType,power);
    end        
    function windowSelect(source, eventData)
        selectedWindow = (get(get(source,'SelectedObject'),'String'));
        if selectedWindow == 'Flat Top'
            selectedWindow = 'flattopwin';
        end
        disp(selectedWindow)
        spectragramRUN(fs, WIND, colOption,windowType);
    end
    function plotSelect(source, eventData)
        selectedPlot = (get(get(source,'SelectedObject'),'String'));
        spectragramRUN(fs, WIND, colOption,windowType,selectedPlot);
    end
